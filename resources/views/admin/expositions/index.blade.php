                ...
                <div class="p-6 sm:p-10">
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                        ...
                        <div class="flex flex-wrap gap-3 w-full sm:w-auto">
                            <x-add-button :action="route('admin.expositions.create')" :text="'Agregar Exposición'" />
                        </div>
                    </div>

                    <form method="GET" action="{{ route('admin.expositions.index') }}" class="mb-6 flex flex-col sm:flex-row gap-3 items-center">
                    ...
                    @if ($expositions->isEmpty())
                        <div class="bg-gray-100 border border-gray-300 rounded-xl p-10 text-center shadow-2xl">
                            ...
                            @unless(request('search'))
                                <p class="text-gray-600 mb-6">Comienza agregando tu primera exposición al sistema</p>
                            @endunless
                        </div>
                    @else
                            ...
                            @foreach ($expositions as $exposition)
                                <div class="bg-white border border-gray-300 rounded-lg shadow p-5 flex flex-col gap-2">
                                    <div class="flex items-center gap-2 mb-2">
                                        <span class="font-semibold text-gray-900 text-lg">Exposición de {{ $exposition->company_name }}</span>
                                    </div>
                                    <div class="text-gray-700 text-sm flex items-center gap-2">
                                        Stand: <span class="font-semibold">{{ $exposition->stand_category }}</span>
                                    </div>
                                    <div class="flex gap-2 mt-2">
                                        <a href="{{ route('admin.expositions.show', $exposition) }}"
                                        ...
                                        <a href="{{ route('admin.expositions.edit', $exposition) }}"
                                        ...
                                        <button type="button"
                                            onclick="openDeleteModal('{{ route('admin.expositions.destroy', $exposition) }}')"
                                ...
                                <tbody class="divide-y divide-gray-200">
                                    @foreach ($expositions as $exposition)
                                        <tr class="hover:bg-[#7692FF]/10 transition-colors duration-200 h-16">
                                            <td class="px-6 py-4 text-sm text-gray-900">{{ $exposition->stand_category }}</td>
                                            <td class="px-6 py-4 text-sm text-gray-900">{{ $exposition->stand_size }}</td>
                                            <td class="px-6 py-4 text-sm text-gray-900">
                                                {{ $exposition->space ? $exposition->space->location_code : '-' }}
                                            </td>
                        ...
                        <x-pagination :paginator="$expositions" />
                    @endif
                </div>
            </div>
        </div>
