            ...
            <div class="bg-white rounded-xl shadow-xl border-2 border-[#7692FF] p-8">
                ...
                @if($eventsCount > 0)
                    <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 3500)"
                        ...
                        <span>No se puede editar el descanso porque ya existe al menos un evento creado.</span>
                    </div>
                @else
                <form method="POST" action="{{ route('admin.schedules.updateBreak', $schedule) }}">
                    @csrf
                    @method('PUT')
                    <div class="mb-4">
                        <label for="break" class="block text-sm font-medium text-gray-700 mb-1">Minutos de descanso entre eventos</label>
                        <input id="break" name="break" class="w-full px-3 py-2 border border-gray-300 rounded-md" value="{{ $schedule->break }}">
                        <span class="text-xs text-gray-500">(Mínimo 5, máximo 15 minutos)</span>
                        @error('break')
